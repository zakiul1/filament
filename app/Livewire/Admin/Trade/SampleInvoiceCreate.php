...
class SampleInvoiceCreate extends Component implements HasSchemas, HasActions
{
    ...
    public function create(): void
    {
        ...
        $sample = SampleInvoice::create($data);

        foreach ($items as $index => $itemData) {
            $itemData['line_no'] = $itemData['line_no'] ?? ($index + 1);
            $itemData['sample_invoice_id'] = $sample->id;

            SampleInvoiceItem::create($itemData);
        }

        session()->flash('success', 'Sample Invoice created successfully.');
    ...
    {
        return view('livewire.admin.trade.sample-invoice-create');
    }
}