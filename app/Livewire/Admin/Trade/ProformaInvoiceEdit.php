...
class ProformaInvoiceEdit extends Component implements HasSchemas, HasActions
{
        ...
        $this->form->fill(array_merge(
            ...
            [
                'items' => $record->items()
                    ->orderBy('line_no')
                    ->get()
                    ->map(function (ProformaInvoiceItem $item) {
                        return $item->only([
                            'line_no',
                            'style_ref',
                            'item_description',
    ...
    public function update(): void
    {
        ...
        // Simplest: delete & re-create items
        $this->record->items()->delete();

        foreach ($items as $index => $itemData) {
            $itemData['line_no'] = $itemData['line_no'] ?? ($index + 1);
            $itemData['proforma_invoice_id'] = $this->record->id;
    ...
    {
        return view('livewire.admin.trade.proforma-invoice-edit');
    }
}